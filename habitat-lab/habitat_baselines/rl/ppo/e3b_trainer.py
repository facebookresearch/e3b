...
class E2BTrainer(BaseRLTrainer):
    ...
    def _init_train(self):
        ...
        ridge = self.config.RL.E2B.ridge
        self.covs = torch.stack([torch.eye(hidden_size) * ridge for i in range(num_envs)]).to(self.device)
        self.inverse_covs = torch.stack([torch.eye(hidden_size) * (1./ridge) \
                                         for i in range(num_envs)]).to(self.device)
        self.outer_product_buffers = torch.empty(num_envs, hidden_size, hidden_size).to(self.device)
        self.elliptical_bonus_buffer = torch.empty(num_envs, 1)
    ...
    def _compute_actions_and_step_envs(self, buffer_index: int = 0):
        ...
        with torch.no_grad():
            if self.config.RL.E2B.embedding == 'idm':
    ...
    def _collect_environment_result(self, buffer_index: int = 0):
        ...
        bonus_rewards = torch.tensor(
            self.elliptical_bonus_buffer.squeeze(),
            dtype=torch.float,
            device=self.current_episode_reward.device,
        )
        ...
        for i, done in enumerate(dones):
            if done:
                self.inverse_covs[i].copy_(torch.eye(self.inverse_covs.shape[1]) * (1./ self.config.RL.E2B.ridge))
    ...
    def _update_agent(self):
        ...
        for i in range(self.config.RL.E2B.inv_dynamics_epochs):
            ...
            phi = self.encoder(obs, None, None, None)[0]
            next_phi = self.encoder(next_obs, None, None, None)[0]
            ...
            while not self.is_done():
                ...
                losses = self._coalesce_post_step(
                    dict(
                        ...
                        inv_dynamics_loss=inv_dynamics_loss,
                    ),
                    count_steps_delta,
                )
